...
class CalendarView(ui.View):
	...
	def __init__(self,fldname,dateval,action=None, grid=[],*args, **kwargs):
		...
		self.d_clr_off=.5, .5, .5, 1.0
		self.bank_total = 0
		self.banked_hours=0
		calendar.setfirstweekday(calendar.SUNDAY)
		self.grid=grid
		self.sum_bank_hours()
		self.banked_val_data=int(self.grid[0][0])
		...
		cv.border_width = 2
		# application buttons
#		cv.right_button_items=(ui.ButtonItem(title='Done',action= self.done_button),)
		self.view = cv
		self.action = action
		...
		banked_value = ui.TextField(name='bankedval',text=str(self.banked_val_data),frame=(155,8*31,100,30), enabled = True, keyboard_type=ui.KEYBOARD_NUMBERS,action=self.banked_val_changed)
		self.view.add_subview(banked_value)
		
		# Bank hours used
		bank_label = ui.Label(name='banktitle',text='Bank Hr Use:',frame=(5,9*31,150,30))
		self.view.add_subview(bank_label)
		
		bank_value = ui.TextField(name='bankval',text=str(self.bank_total),frame=(155,9*31,100,30), enabled = False)
		self.view.add_subview(bank_value)
		
		#remaining bank hours
		bank_remain_label = ui.Label(name='remaintitle',text='Bank Hr Remain:',frame=(5,10*31,150,30))
		self.view.add_subview(bank_remain_label)
		
		bank_remain_value = ui.TextField(name='remainval',text=str(self.banked_val_data-self.bank_total),frame=(155,10*31,100,30), enabled = False)
...
#				print('black')
	...
	def button_pressed(self,sender):
		...
		'''
		# works, but effects all months?!
		if 	(sender.background_color == self.d_clr_none or sender.background_color == self.d_clr_off):
